...
class Dictionary
  ...
  open('dics/pattern.txt') do |f|
    ...
    pattern, phrases = line.encode!("utf-8").chomp.split("\t")
...
def initialize(pattern, phrases)
  SEPARATOR =~ pattern.encode!("utf-8")
    # pattern内に数値＋文字列のパターンが入っているか
    # 後方参照でパターンマッチを行う

    @modify, @pattern = $2.to_i, $3.to_s
...
@dictionary.pattern.each do |ptn_item|
      ...
      if ptn_item.match(input)
        ...
        adjust_mood(ptn_item.modify.to_i)
...
m = Mecab.new("")
 ...
 # puts m.sparse_tostr(input)
  ...
  # 戻り値をresponseという変数に代入
    ...
    puts(prompt(proto) + response.encode!("utf-8"))
